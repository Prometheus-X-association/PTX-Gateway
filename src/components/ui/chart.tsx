...
const sanitizeColorValue = (color: string | undefined): string | null => {
  ...
  // Allow hex, rgb, rgba, hsl, hsla, oklch, and CSS named colors only
  const validPatterns = [
  ...
  return validPatterns.some(pattern => pattern.test(color.trim())) ? color : null;
};

// Sanitize keys to alphanumeric, underscore, hyphen only
